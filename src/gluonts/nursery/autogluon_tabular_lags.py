...
def get_prediction_dataframe(series, prediction_length, context_length, future_series=None, train=False):
    if not train:
...
class TabularPredictor(Predictor):
    def __init__(self, ag_model, freq: str, prediction_length: int, context_length: int) -> None:
    ...
    def predict(self, dataset: Iterable[Dict]) -> Iterator[SampleForecast]:
        for entry in dataset:
            ...
            df = get_prediction_dataframe(ts, self.prediction_length, self.context_length, future_ts, train=False)
...
class TabularEstimator(Estimator):
    def __init__(self, freq: str, prediction_length: int, context_length: int) -> None:
        super().__init__()
        self.task = task
        self.freq = freq
        self.prediction_length = prediction_length
    ...
    def train(self, training_data: Dataset) -> TabularPredictor:
        ...
        dfs = [
            get_prediction_dataframe(to_pandas(entry), self.prediction_length, self.context_length, train=True)
            for entry in training_data
        ]
        df = pd.concat(dfs)
        ag_model = self.task.fit(df, label="target")
        return TabularPredictor(ag_model, self.freq, self.prediction_length, self.context_length)
